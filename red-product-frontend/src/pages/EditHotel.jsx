...
function EditHotel() {
  ...
  useEffect(() => {
    const fetchHotelDetails = async () => {
      ...
      try {
        ...
        setEmail(data.email || '')
        setPhone(data.phone || '')
        setPrice(data.price_per_night ? Math.round(parseFloat(data.price_per_night)).toString() : '')
        
        // Formater la devise stockée vers le label du sélecteur
        const formattedCurrency = data.currency === 'XOF' ? 'F XOF' : data.currency === 'EUR' ? 'Euro' : 'Dollar'
        setCurrency(formattedCurrency)
        setPreviewUrl(data.image || '')
      } catch (err) {
        console.error("Erreur lors du chargement de l'hôtel :", err)
        setError("Impossible de charger les détails de cet hôtel.")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/hotels/${id}/`, payload, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
      ...
      const errorMsg = err.response?.data?.name?.[0] || 
                       ...
                       "Impossible de modifier l'hôtel. Veuillez vérifier les champs."
      setError(errorMsg)
    } finally {
      setSaving(false)
    }
  ...
  return (
    <DashboardLayout pageTitle="Modifier l'hôtel">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <span className="text-gray-500 animate-pulse text-sm">Chargement des détails de l'hôtel...</span>
        </div>
      ) : (
